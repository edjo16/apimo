import { TDataHeaderQuotation, TExportPDF, TItem, TQuotation, TQuotationDataForm, TTax, TypeLine } from './Type';
import { jsPDF } from 'jspdf';
import autoTable from 'jspdf-autotable';
import moment from 'moment';
import Big from 'big.js';

export const validateInput = ({ header, items }: TQuotationDataForm) => {
  if (!header || !items) {
    throw new Error('Faltan parámetros');
  }
  const requiredProps: (keyof TDataHeaderQuotation)[] = ['client', 'code', 'contact', 'expirationDate', 'salesEmployee'];

  for (const prop of requiredProps) {
    if (!(prop in header)) {
      throw new Error(`${prop} no encontrado`);
    }
  }
...
export const ordenateItems = async (items: any, productService: any, taxService: any): Promise<TItem[]> => {
  const itemsFormat: TItem[] = [];
  const taxs = await getTaxs(taxService);

  for (const item of items) {
    if (item.type === TypeLine.PRODUCT) {
      const product = await productService.readOne(item.number, {
        fields: ['code'],
      });

      if (!product) throw new Error('Codigo no encontrado ');

      const tax = taxs.find((tax) => tax.code === item.tax);

      if (!tax) throw new Error('ITBMS no encontrado ');

      itemsFormat.push({ ...item, productId: product.code, taxId: tax.id });
    } else if (item.type === TypeLine.LINE) {
      itemsFormat.push({
        type: item.type,
...
export const getTaxs = async (taxService: any): Promise<TTax[]> => {
  const tax: TTax[] = await taxService.readByQuery({
...
export const calculate = async (data: TQuotation, taxService: any) => {
  const taxs = await getTaxs(taxService);

  return data.items.reduce(
    (acc, current) => {
      const { qty, price, tax } = current;

      if (!qty || !price || !tax) return acc;

      const findTax = taxs.find((t) => t.code === tax);

      if (findTax) {
        const priceLine = Big(qty).times(Big(price));
        const taxRate = Big(findTax.value).div(100);
